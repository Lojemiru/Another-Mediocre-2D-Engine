...
public class BetterTextureFragment(Texture2D texture, Rectangle region, int verticalOffset = 0) : IRenderable
{
    ...
    {
        context.DrawImage(Texture, Region, new Vector2(position.X, position.Y + VerticalOffset + EngineCore.DefaultImageEmbedVerticalOffset), Scale, color);
    }
}